  ...
  if (isUser) {
    return (
      <div className="flex justify-end">
        <div className="max-w-[80%] rounded-2xl rounded-tr-sm bg-zinc-900 text-white px-4 py-2.5 text-sm">
          {message.content}
        </div>
      </div>
    );
      ...
      <div className="prose prose-sm max-w-none text-zinc-800">
        <ReactMarkdown
          ...
          components={{
            ...
            h3: ({ children }) => <h3 className="text-sm font-semibold mt-2 mb-1 text-zinc-800">{children}</h3>,
            // Código inline
            code: ({ children }) => (
            ...
            // Bloco de código
            pre: ({ children }) => (
              <pre className="bg-zinc-100 rounded-md p-3 overflow-x-auto text-xs font-mono my-2">
                {children}
              </pre>
            ),
          }}
